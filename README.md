...
> 本人是大一新生，计算机小白，接触 agent 才两周左右，诸多不足都可以提出来，目前还在摸索阶段。
> 
> 本人特别喜欢鸣潮，所以桌面端的图标使用的是鸣潮的绯雪，自己有喜欢的可以更换。往后打算做鸣潮主题的dsh，目前正在开发。

😊这是一个早期版本，基本上是 dsh 的白板桌面端。起因是个人在使用 dsh 时觉得网页端有些麻烦、不够个性化，就心血来潮做个桌面端。后又刷到 B 站「AI 猫娘」计划，就干脆将此做个跳板，日后自己做个桌面小鲸鱼，目前正在努力开发。

😘关于我对dsh的开发理念---最自由+最稳定
    将最多的自由权交给用户自己，背景/字体/开机/样式，任何可以个性化的，都可以自定义，同时最大限度保障运行稳定。每次发布都经过作者多次测试，同时增加备份功能。

😎把 dsh 装进一个真正的 Windows 桌面窗口。

😍不用开终端敲命令、不用手动管端口、不用记 URL——双击就进。

😉自动更新：不用担心跟不上版本，dsh拥有检测更新按钮，一键完成更新。同时作者每日高强度更新。

> **difish 只是壳。** 真正的能力来自 `dsh`；difish 负责把它拉起来、给它一个好看的窗口、加上托盘/快捷键/背景这些桌面端该有的东西。

---![Uploading dfa8b460fc43666d2af18623ea4c64f7.png…]()
...
| 🌊 **开机画面** | 后端启动要 10–90 秒，这段等待期不再是一片黑：全屏开机画面 + 进度条 + **真实已等待秒数**。支持放自己的 mp4（丢进 `assets/boot.mp4` 即可），没有素材时自动显示字标，不会白屏 |
